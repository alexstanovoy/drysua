...
После разрешения от 2026-09-08 действует общий принцип экспериментов:
сначала дешёвая проверка гипотезы, затем полноценная инвестиция времени.
Не продолжать длинный запуск только потому, что предыдущий уже был дорогим.

Актуальное уточнение от пользователя: production-обучение теперь **только Map2**,
mid-only, с 15 игровыми минутами и прежним pregame; старые описания Map0 ниже
сохраняют историю разработки, а не задают текущий training target.
После зависания компьютера обязательны
[последовательные тяжёлые запуски с ограничениями памяти и watchdog](experiment-safety.md).
Текущие замеры CPU-конфигурации и их ограничения:
[X3D Turbo](../artifacts/temp/x3d-turbo-20260912/REPORT.md).
