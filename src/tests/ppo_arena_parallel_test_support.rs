...
#[cfg(test)]
pub(crate) fn ordered_active<T: Send, I: Send, O: Send>(
    worlds: &mut [T],
    jobs: Vec<(usize, I)>,
    operation: impl Fn(usize, &mut T, I) -> Result<O, PpoError> + Sync,
) -> Result<Vec<O>, PpoError> {
    if worlds.len() > super::super::TRAINING_MAX_ENVIRONMENTS
    ...
    assert!(jobs.len() <= super::super::TRAINING_MAX_ENVIRONMENTS);
    if jobs.len() <= 1 {
        return jobs
            .into_iter()
...
fn join_ordered<O>(
    ...
    assert!(handles.len() <= super::super::TRAINING_MAX_ENVIRONMENTS);
    ...
    assert!(output.len() <= super::super::TRAINING_MAX_ENVIRONMENTS);
